...
const reviews: Review[] = [
  {
    quote:
      "Before working with Inga, math homework was a nightly battle. Now, my daughter actually looks forward to her sessions!",
  ...
  {
    quote:
      "Inga has a gift for making science concepts click. My son's confidence in physics has skyrocketed.",
  ...
  {
    quote:
      "Small group setting is perfect. Individual attention without the intimidation of 1-on-1 tutoring.",
...
const ReviewsSection = () => {
  return (
    <section
      id="reviews"
      className="py-10 sm:py-12 md:py-16 lg:py-20 bg-primary/5 relative z-10"
    >
      <div className="max-w-6xl mx-auto px-4 sm:px-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-5 md:gap-6">
          {reviews.map((review, i) => (
            <article
              key={i}
              className="bg-card p-4 sm:p-5 md:p-6 lg:p-8 rounded-xl sm:rounded-2xl shadow-sm border border-border relative card-hover"
            >
              <MessageCircle
                className="absolute top-3 right-3 sm:top-4 sm:right-4 md:top-6 md:right-6 text-primary/20 w-5 h-5 sm:w-6 sm:h-6 md:w-8 md:h-8 lg:w-10 lg:h-10"
                aria-hidden="true"
              />
              <div className="flex text-primary mb-2 sm:mb-3 md:mb-4" aria-label="5 star rating">
                {[1, 2, 3, 4, 5].map((s) => (
                  <Star key={s} size={12} className="sm:w-3.5 sm:h-3.5" fill="currentColor" />
                ))}
              </div>
