      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-5 md:gap-6">
          {reviews.map((review, i) => <article key={i} className="bg-card p-4 sm:p-5 md:p-6 lg:p-8 rounded-xl sm:rounded-2xl shadow-sm border border-border relative card-hover">
              <MessageCircle aria-hidden="true" className="absolute top-3 right-3 sm:top-4 sm:right-4 md:top-6 md:right-6 w-5 h-5 sm:w-6 sm:h-6 md:w-8 md:h-8 lg:h-[36px] lg:w-[36px] text-purple-500" />
              <div className="flex text-primary mb-2 sm:mb-3 md:mb-4" aria-label="5 star rating">
                {[1, 2, 3, 4, 5].map(s => <Star key={s} size={12} fill="currentColor" className="sm:w-3.5 sm:h-3.5 text-pink-600" />)}
              </div>
              ...
              <footer className="flex items-center gap-2 sm:gap-3">
                <div className="w-7 h-7 sm:w-8 sm:h-8 md:w-10 md:h-10 rounded-full flex items-center justify-center font-bold text-xs sm:text-sm bg-orange-300 text-pink-600">
                  {review.initial}
                </div>
                <div>
