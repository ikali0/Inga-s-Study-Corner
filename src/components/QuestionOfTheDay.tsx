...
const QuestionOfTheDay = () => {
  ...
  return (
    <section className="py-6 sm:py-12 px-4 font-sans w-full max-w-4xl mx-auto">
      <style>{`
        @keyframes shake {
      ...
      <div className="w-full">
        <Card className="overflow-hidden border-2 border-slate-100 shadow-xl bg-white transition-all duration-300">
          {/* Status Bar */}
          <div
            className={`h-2 w-full transition-colors duration-500 ${status === "correct" ? "bg-green-500" : status === "wrong" ? "bg-red-500" : "bg-gradient-to-r from-blue-500 via-purple-500 to-pink-500"}`}
          />
          ...
          <CardHeader>
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
              <div className="flex items-center gap-2">
                ...
                <CardTitle className="text-slate-800">{isDaily ? "Question of the Day" : "Practice Mode"}</CardTitle>
              </div>

              <div
                className={`self-start sm:self-auto inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs sm:text-sm font-semibold border ${style.badge}`}
              >
                {style.icon}
                {style.label}
              </div>
          ...
          <CardContent className="space-y-6">
            ...
            <div className="bg-slate-50 rounded-xl p-5 sm:p-6 border border-slate-100 min-h-[100px] flex items-center justify-center text-center py-[8px] px-[8px]">
              <p className="text-lg sm:text-xl font-medium text-slate-800 leading-relaxed">{activeQuestion.question}</p>
            </div>

            {/* Options Grid */}
            <div
              className={`grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4 ${status === "wrong" ? "animate-shake" : ""}`}
            >
              {activeQuestion.options.map((option, index) => {
                ...
                let classes =
                  "relative w-full p-4 rounded-xl border-2 text-left font-medium transition-all duration-200 flex items-center justify-between min-h-[56px] active:scale-[0.98] ";
                if (isDisabled) {
                  ...
                  else classes += "bg-slate-50 border-transparent text-slate-400 opacity-50 ";
                } else {
                  classes +=
            ...
            <div className="flex flex-col items-center gap-4 pt-2">
              ...
              {status === "unanswered" && (
                <>
                  ...
                  {showHint && (
                    <div className="w-full bg-yellow-50 border border-yellow-200 rounded-lg p-3 text-sm text-yellow-800 animate-in fade-in slide-in-from-top-2 text-center">
                      💡 {activeQuestion.hint}
                    </div>
                  )}
                </>
              ...
              {status !== "unanswered" && (
                <div className="w-full space-y-4 animate-in zoom-in-95 duration-300">
                  <div
                    className={`p-4 rounded-xl text-center ${status === "correct" ? "bg-green-50 border border-green-100" : "bg-red-50 border border-red-100"}`}
                  >
                  ...
                  <div className="flex flex-col sm:flex-row gap-3 justify-center min-w-full">
                    {!isDaily && status === "wrong" && (
                      <Button
                        ...
                        className="w-full sm:w-auto border-slate-300 h-12 sm:h-10 text-base sm:text-sm"
                      >
                    ...
                    <Button
                      ...
                      className="w-full sm:w-auto bg-slate-900 text-white hover:bg-slate-800 h-12 sm:h-10 text-base sm:text-sm"
                    >
