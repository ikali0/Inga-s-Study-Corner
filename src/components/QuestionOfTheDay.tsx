...
const QuestionOfTheDay = () => {
  ...
  return <div className="w-full font-sans">
      ...
      <div className="bg-white rounded-xl border-2 border-slate-900 shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] overflow-hidden">
        {/* Status Bar */}
        <div className={cn("h-1 w-full transition-colors duration-300", status === "correct" ? "bg-green-500" : status === "wrong" ? "bg-red-500" : "bg-gradient-to-r from-blue-500 via-purple-500 to-pink-500")} />
        ...
        <div className="p-4">
          ...
          <div className="flex items-center justify-between gap-2 mb-3">
            ...
            <div className={cn("inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-semibold border", style.badge)}>
              {style.icon}
              {style.label}
            </div>
          </div>
          ...
          {/* Options - 2x2 Grid */}
          <div className={cn("grid grid-cols-2 gap-2 mb-3", status === "wrong" ? "animate-shake" : "")}>
            {activeQuestion.options.map((option, index) => {
            const isSelected = selectedAnswer === index;
            const isCorrect = index === activeQuestion.correctIndex;
            const isDisabled = status !== "unanswered";
            return <button key={index} onClick={() => handleAnswerSelect(index)} disabled={isDisabled} className={cn("relative p-2.5 rounded-lg border-2 text-left text-xs font-medium transition-all duration-150 flex items-center justify-between min-h-[40px]", isDisabled ? isCorrect ? "bg-green-50 border-green-500 text-green-700" : isSelected ? "bg-red-50 border-red-500 text-red-700" : "bg-slate-50 border-slate-200 text-slate-400 opacity-50" : "bg-white border-slate-200 hover:border-blue-400 hover:bg-blue-50 cursor-pointer text-slate-700 active:scale-[0.98]")}>
          ...
          <div className="flex flex-col gap-2">
            {status === "unanswered" && <>
                <Button variant="ghost" size="sm" onClick={() => setShowHint(!showHint)} className="text-blue-600 hover:bg-blue-50 h-8 text-xs w-full">
                  <Lightbulb className={cn("w-3 h-3 mr-1", showHint && "fill-blue-600")} />
                  {showHint ? "Hide Hint" : "Need a Hint?"}
                </Button>
                {showHint && <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-2 text-xs text-yellow-800 text-center">
                    💡 {activeQuestion.hint}
                  </div>}
              </>}

            {status !== "unanswered" && <div className="space-y-2">
                <div className={cn("p-2.5 rounded-lg text-center", status === "correct" ? "bg-green-50 border border-green-100" : "bg-red-50 border border-red-100")}>
                  <p className={cn("text-sm font-bold", status === "correct" ? "text-green-800" : "text-red-800")}>
                ...
                <div className="flex gap-2">
                  {!isDaily && status === "wrong" && <Button variant="outline" size="sm" onClick={() => setStatus("unanswered")} className="flex-1 h-8 text-xs border-slate-300">
                      <RotateCcw className="w-3 h-3 mr-1" /> Retry
                    </Button>}
                  <Button size="sm" onClick={handleNextQuestion} className="flex-1 bg-slate-900 text-white hover:bg-slate-800 h-8 text-xs">
