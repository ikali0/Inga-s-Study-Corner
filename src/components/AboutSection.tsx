...
const AboutSection = () => {
  return <section id="about" className="py-8 sm:py-12 md:py-16 lg:py-20 relative z-10">
        ...
        <div className="bg-secondary rounded-xl sm:rounded-2xl text-secondary-foreground p-5 sm:p-6 md:p-10 lg:p-12 relative overflow-hidden">
          ...
          <div className="relative z-10">
            ...
            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
              {features.map((feature, index) => <article key={index} className="flex gap-3 p-4 rounded-lg bg-card/90 border border-border/50 backdrop-blur-sm transition-shadow hover:shadow-md">
                  <div className={`${feature.iconBg} p-2.5 rounded-lg h-fit shrink-0`}>
                    {feature.icon}
                  </div>
                  <div>
                    <h3 className="text-sm font-bold mb-1 text-card-foreground sm:text-sm">
                      {feature.title}
                    </h3>
                    <p className="text-xs text-muted-foreground leading-relaxed sm:text-xs">
                      {feature.description}
                    </p>
                  </div>
                </article>)}
