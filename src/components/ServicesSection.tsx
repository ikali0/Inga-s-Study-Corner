...
const services: Service[] = [{
  ...
  longDescription: "Build strong math foundations with visual manipulatives, real-world problems, and confidence-building techniques.",
  detailedFeatures: [{
    title: "K-2 Focus",
    items: ["Number sense", "Addition & subtraction", "Basic geometry"]
  }, {
    title: "3-5 Focus",
    items: ["Multiplication & division", "Fractions", "Word problems"]
  }, {
    title: "6-8 Focus",
    items: ["Pre-algebra", "Ratios", "Geometry", "Data analysis"]
  }],
  ...
  longDescription: "Supports full journey from learning to read to reading to learn, building fluent readers and confident writers.",
  detailedFeatures: [{
    title: "K-2 Focus",
    items: ["Phonics", "Sight words", "Early comprehension"]
  }, {
    title: "3-5 Focus",
    items: ["Main idea", "Paragraph structure", "Vocabulary in context"]
  }, {
    title: "6-8 Focus",
    items: ["Literary analysis", "Argumentative essays", "Research skills"]
  }],
  ...
  detailedFeatures: [{
    title: "K-2 Focus",
    items: ["Community helpers", "Map skills", "Holidays"]
  }, {
    title: "3-5 Focus",
    items: ["U.S. history", "States & capitals", "Primary sources"]
  }, {
    title: "6-8 Focus",
    items: ["World history", "Document analysis", "Civics"]
  }],
  ...
  detailedFeatures: [{
    title: "Life Science",
    items: ["Ecosystems", "Human body systems", "Plant & animal cycles"]
  }, {
    title: "Physical Science",
    items: ["Matter & energy", "Forces & motion", "Light & sound"]
  }, {
    title: "Earth & Space",
    items: ["Solar system", "Weather patterns", "Geology"]
  }, {
    title: "Digital Literacy",
    items: ["Scratch coding", "Logical sequencing", "Robotics intro"]
  }],
...
const CardFront = ({
  service,
  theme
}: CardFrontProps) => <div className="flex flex-col h-full p-3 sm:p-4">
    {/* Header with icon */}
    <div className="flex items-center gap-2 mb-2">
      ...
      <span className={`px-1.5 sm:px-2 py-0.5 text-[8px] sm:text-[9px] font-bold uppercase rounded-full ${theme.badge}`}>
        K-8
      </span>
    </div>
    
    {/* Title & Description */}
    <h3 className="font-bold text-foreground text-xs sm:text-sm md:text-base mb-1 leading-tight">
      <span className="hidden sm:inline">{service.title}</span>
      <span className="sm:hidden">{service.shortTitle}</span>
    </h3>
    <p className="text-muted-foreground text-[10px] sm:text-xs leading-relaxed mb-2 line-clamp-2">
      {service.description}
    </p>
    
    {/* Features - show fewer on mobile */}
    <ul className="space-y-1 mb-2 flex-grow">
      {service.features.slice(0, 2).map((f, i) => <li key={i} className="flex items-center gap-1.5">
          <CheckCircle2 className={`w-2.5 h-2.5 sm:w-3 sm:h-3 shrink-0 ${theme.check}`} />
          <span className="text-foreground text-[9px] sm:text-[11px] font-medium truncate">{f}</span>
        </li>)}
      <li className="hidden sm:flex items-center gap-1.5">
        <CheckCircle2 className={`w-3 h-3 shrink-0 ${theme.check}`} />
        <span className="text-foreground text-[11px] font-medium truncate">{service.features[2]}</span>
      </li>
    </ul>
    
    {/* Hover/Tap hint */}
    <div className="flex items-center justify-center gap-1 text-[9px] sm:text-[10px] text-muted-foreground mt-auto pt-2 border-t border-border/50">
      <span className="hidden sm:inline">Hover for more</span>
...
const CardBack = ({
  ...
  theme,
  onLearnMore
}: CardBackProps) => <div className={`flex flex-col items-center justify-center h-full p-3 sm:p-4 bg-gradient-to-br ${theme.gradient}`}>
    <div className={`w-8 h-8 sm:w-10 sm:h-10 rounded-full flex items-center justify-center ${theme.iconBg} mb-2`}>
      {React.cloneElement(service.icon, {
      className: "w-4 h-4 sm:w-5 sm:h-5"
    })}
    </div>
    ...
    <p className="text-muted-foreground text-[9px] text-center leading-relaxed mb-2 line-clamp-2 sm:line-clamp-3 px-1 sm:text-xs">
      {service.longDescription}
    </p>
    
    <div className="text-center mb-2 sm:mb-3 hidden sm:block">
      <p className="text-[9px] sm:text-[10px] font-semibold text-foreground mb-0.5">Our Approach:</p>
      <p className="text-[9px] sm:text-[10px] text-muted-foreground line-clamp-2 px-1">{service.approach}</p>
    </div>
    
    <Button onClick={e => {
    e.stopPropagation();
    onLearnMore();
  }} className={`${theme.button} text-primary-foreground font-bold text-[10px] sm:text-xs px-2.5 sm:px-3 py-1 sm:py-1.5 h-6 sm:h-7`}>
      View Details
    </Button>
  </div>;
...
const DetailedFeatures = ({
  ...
  checkClass: string;
    ...
    {sections.map((section, idx) => <div key={idx} className="p-3 sm:p-4 rounded-lg bg-card border border-border">
...
const OutcomesList = ({
  ...
  outcomes: string[];
    ...
    {outcomes.map((outcome, i) => <li key={i} className="flex items-start gap-2 sm:gap-3 p-2.5 sm:p-3 rounded-lg bg-primary/5 border border-primary/20">
...
const ServicesSection: React.FC = () => {
  ...
  const gradientClasses = ["from-primary/10 via-blue/5 to-purple/10", "from-purple/10 via-primary/5 to-orange/10", "from-orange/10 via-green/5 to-blue/10", "from-blue/10 via-purple/5 to-primary/10"];
  const handleBooking = () => {
  ...
  return <section className="py-12 sm:py-16 md:py-20 lg:py-24 relative overflow-hidden" id="services">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 sm:gap-4 md:gap-5 lg:gap-6">
          ...
          return <FlippingCard key={service.id} className={`${theme.card} shadow-lg hover:shadow-xl transition-shadow duration-300`} frontContent={<CardFront service={service} theme={theme} />} backContent={<CardBack service={service} theme={theme} onLearnMore={() => setSelectedService(service)} />} />;
        })}
        </div>
      ...
      <Sheet open={!!selectedService} onOpenChange={open => !open && setSelectedService(null)}>
        <SheetContent side="right" className="w-full sm:max-w-md p-0 h-[100dvh] flex flex-col">
          {selectedService && <ScrollArea className="flex-1 overflow-y-auto">
              <div className="p-4 sm:p-6 md:p-8">
                <SheetHeader className="mb-4 sm:mb-6">
                  <div className="flex items-center gap-3 sm:gap-4 mb-3 sm:mb-4">
                    <div className={`w-10 h-10 sm:w-14 sm:h-14 rounded-xl flex items-center justify-center ${themeConfig[selectedService.color].iconBg}`}>
                      {React.cloneElement(selectedService.icon, {
                    className: "w-5 h-5 sm:w-7 sm:h-7"
                  })}
                    </div>
                    <div className={`px-2 sm:px-3 py-0.5 sm:py-1 text-[10px] sm:text-xs font-bold uppercase rounded-full ${themeConfig[selectedService.color].badge}`}>
                      Grades K-8
                    </div>
                  </div>
                ...
                <section className="mb-4 sm:mb-6">
                  ...
                  <DetailedFeatures sections={selectedService.detailedFeatures} checkClass={themeConfig[selectedService.color].check} />
                </section>
                ...
                <Button onClick={handleBooking} className={`w-full py-5 sm:py-6 text-sm sm:text-base font-bold ${themeConfig[selectedService.color].button} text-primary-foreground`}>
