          ...
          {showSuccess ? <div className="text-center py-12 animate-in zoom-in">
              ...
              <div className="grid md:grid-cols-2 gap-6">
                <div className="space-y-2">
                  ...
                  <Input required type="text" placeholder="Jane Doe" value={formData.parentName} onChange={e => handleInputChange('parentName', e.target.value)} className={`bg-muted border-border ${errors.parentName ? 'border-destructive' : ''}`} maxLength={100} />
                ...
                <div className="space-y-2">
                  ...
                  <Input required type="text" placeholder="Leo, Age 10" value={formData.childName} onChange={e => handleInputChange('childName', e.target.value)} className={`bg-muted border-border ${errors.childName ? 'border-destructive' : ''}`} maxLength={100} />
              ...
              <div className="grid md:grid-cols-2 gap-6">
                <div className="space-y-2">
                  ...
                  <Input required type="email" placeholder="hello@family.com" value={formData.email} onChange={e => handleInputChange('email', e.target.value)} className={`bg-muted border-border ${errors.email ? 'border-destructive' : ''}`} maxLength={255} />
                ...
                <div className="space-y-2">
                  ...
                  <Input type="tel" placeholder="(215) 555-0123" value={formData.phone} onChange={e => handleInputChange('phone', e.target.value)} className={`bg-muted border-border ${errors.phone ? 'border-destructive' : ''}`} maxLength={20} />
              ...
              <div className="space-y-2">
                ...
                <Textarea placeholder="Tell me a bit about what your child is working on or struggling with..." value={formData.message} onChange={e => handleInputChange('message', e.target.value)} className={`bg-muted border-border min-h-[120px] resize-none ${errors.message ? 'border-destructive' : ''}`} maxLength={1000} />
                {errors.message && <p className="text-destructive text-xs ml-1">{errors.message}</p>}
              </div>

              <Button type="submit" size="lg" disabled={isSubmitting} className="w-full bg-gradient-to-r from-blue to-accent hover:opacity-90 text-primary-foreground font-bold py-4 rounded-xl shadow-lg transform transition-transform hover:scale-[1.01] active:scale-[0.98] disabled:opacity-70">
                {isSubmitting ? <>
