...
const Footer = ({ onNavigate }: FooterProps) => {
  return (
    <footer className="bg-secondary text-secondary-foreground py-12 md:py-16 relative z-10 overflow-hidden">
      {/* Top gradient bar */}
      <div className="absolute top-0 left-0 w-full h-1 bg-gradient-to-r from-primary via-accent to-pink"></div>
      
      <div className="max-w-6xl mx-auto px-4 grid md:grid-cols-2 lg:grid-cols-4 gap-8 md:gap-10">
        
        {/* Brand & Mission */}
        <div className="lg:col-span-1">
          <h3 className="text-primary-foreground font-bold text-xl md:text-2xl mb-4 flex items-center gap-2">
          ...
          <div className="flex items-center gap-2 text-sm text-secondary-foreground/70">
            ...
            <span>Tue-Fri • 3:00-9:00 PM</span>
          </div>
        </div>

        {/* Parent Trust Column */}
        <div className="lg:col-span-1">
          <h4 className="text-primary-foreground font-bold mb-4 text-lg">Parent Peace of Mind</h4>
          <ul className="space-y-3">
            <li className="flex items-start gap-3">
              <ShieldCheck className="text-green shrink-0" size={20} />
              <span className="text-sm text-secondary-foreground/80">Safe & Vetted Environment</span>
            </li>
            <li className="flex items-start gap-3">
              <GraduationCap className="text-blue shrink-0" size={20} />
              <span className="text-sm text-secondary-foreground/80">Patient, Expert Guidance</span>
            </li>
            <li className="flex items-start gap-3">
              <Clock className="text-purple shrink-0" size={20} />
              <span className="text-sm text-secondary-foreground/80">Flexible After-School Hours</span>
            </li>
          </ul>
        </div>
        ...
        {/* Quick Links */}
        <div className="lg:col-span-1">
          <h4 className="text-primary-foreground font-bold mb-4 text-lg">Explore</h4>
          <ul className="space-y-2 text-sm">
            ...
            <li>
              <button onClick={() => onNavigate('about')} className="text-secondary-foreground/70 hover:text-primary transition-colors">
                Why Parents Love It
              </button>
            </li>
            <li>
              <button onClick={() => onNavigate('reviews')} className="text-secondary-foreground/70 hover:text-primary transition-colors">
                Parent Reviews
              </button>
            </li>
            <li>
              <button onClick={() => onNavigate('book')} className="text-secondary-foreground/70 hover:text-primary transition-colors">
                Book Consultation
              </button>
            </li>
          </ul>
        ...
        <div className="lg:col-span-1" id="contact">
          ...
          <div className="bg-secondary/50 p-4 rounded-xl space-y-3 border border-secondary-foreground/10">
            <a href={`mailto:${tutorInfo.email}`} className="flex items-center gap-3 text-sm text-secondary-foreground/80 hover:text-primary-foreground transition-colors">
              ...
              <span className="break-all">{tutorInfo.email}</span>
            </a>
            ...
            <div className="flex items-start gap-3 text-sm text-secondary-foreground/80">
              ...
              <span>{tutorInfo.location}<br/><span className="text-xs text-secondary-foreground/60">{tutorInfo.address}</span></span>
            </div>
          </div>
        </div>
      </div>
